...
def control_action(text): ##명령어 입력
	open_1door1 = '1번 열어'
	open_1door2 = '1번 열어줘'
	open_1door3 = '1번 서랍 열어'
	open_1door4 = '1번 서랍 열어줘'
	#close_can = '닫아'.decode('utf-8')
	#close_can2 = '닫아줘'.decode('utf-8')
	#cmd_init()
	
	if (open_1door1 in text or open_1door2 in text):
		print('1번 서랍을 열겠습니다라')
		gt2vt.getText2VoiceStream("1번 서랍을 열겠습니다.다","./number1_open.wav")
...
def main():
	...
	while 1:
		...
		if recog == 200:
			gt2vt.getText2VoiceStream("AC를 실행합니다.다","./AC_openning.wav")
			...
			while 1:
				if retry_game:
					...
					control_action(text)
					if(say_exit in text):
						gt2vt.getText2VoiceStream("AC를 종료합니다. 감사합니다.다","./AC_closing.wav")
